...
impl DebugAdapter for GoDebugAdapter {
    ...
    fn dap_schema(&self) -> serde_json::Value {
        ...
        let common_properties = json!({
            ...
            "showLog": {
                "type": "boolean",
                "description": "显示 delve 调试器的日志输出。映射到 dlv 的 `--log` 标志。",
                "default": false
            },
            "cwd": {
                "type": "string",
            ...
            "dlvFlags": {
                "type": "array",
                "description": "`dlv` 的额外标志。有关受支持标志的完整列表，请参阅 `dlv help`。",
                "items": {
                    "type": "string"
                },
                "default": []
            ...
            "port": {
                "type": "number",
                "description": "调试服务器端口。对于远程配置，这是连接的位置。",
                "default": 2345
            },
            "host": {
                "type": "string",
                "description": "调试服务器主机。对于远程配置，这是连接的位置。",
                "default": "127.0.0.1"
            },
            "substitutePath": {
                "type": "array",
                "items": {
                    ...
                    "properties": {
                        "from": {
                            "type": "string",
                            "description": "要替换的绝对本地路径。"
                        },
                        "to": {
                            "type": "string",
                            "description": "要替换的绝对远程路径。"
                        }
                    }
                },
                "description": "从本地路径到远程路径的映射以进行调试。",
                "default": []
            },
            "trace": {
                "type": "string",
            ...
            "backend": {
                ...
                "description": "delve 使用的后端。映射到 `dlv` 的 `--backend` 标志。"
            },
            "logOutput": {
                "type": "string",
                "enum": ["debugger", "gdbwire", "lldbout", "debuglineerr", "rpc", "dap"],
                "description": "应该产生调试输出的组件。",
                "default": "debugger"
            },
            "logDest": {
                "type": "string",
                "description": "delve 的日志目标。"
            },
            "stackTraceDepth": {
                "type": "number",
            ...
            "hideSystemGoroutines": {
                "type": "boolean",
                "default": false,
                "description": "从调用堆栈视图中隐藏系统 goroutine。"
            },
            "console": {
                "default": "internalConsole",
                "description": "在哪里启动调试器。",
                "enum": ["internalConsole", "integratedTerminal"]
            },
            "asRoot": {
                "default": false,
                "description": "使用提升的权限进行调试（在 Unix 上）。",
                "type": "boolean"
            }
        });
        ...
        let launch_properties = json!({
            ...
            "traceDirPath": {
                "type": "string",
                "description": "记录跟踪的目录（用于“重播”模式）。",
                "default": ""
            },
            "coreFilePath": {
                "type": "string",
                "description": "核心转储文件路径（用于 'core' 模式）。",
                "default": ""
            }
        });
    ...
    async fn get_binary(
        ...
        let delve_path = if let Some(path) = user_installed_path {
            ...
            let go = delegate
                .which(OsStr::new("go"))
                .await
                .context("PATH 中未找到 Go。请先安装 Go，随后会自动安装 Dlv。")?;

...
impl Pane {
    ...
    pub async fn save_item(
        ...
        const CONFLICT_MESSAGE: &str = "此文件在你开始编辑后已在磁盘上更改。要覆盖它吗？";

        const DELETED_MESSAGE: &str = "此文件在你开始编辑后已从磁盘删除。要重新创建它吗？";
        ...
        let tab = Tab::new(ix)
            ...
            .map(|this| {
                ...
                let end_slot = if is_pinned {
                    ...
                    IconButton::new("取消固定标签页", IconName::Pin)
                        .shape(IconButtonShape::Square)
                        .icon_color(Color::Muted)
                        .size(ButtonSize::None)
                        .icon_size(IconSize::Small)
                    ...
                    match show_close_button {
                        ShowCloseButton::Always => IconButton::new("关闭标签页", IconName::Close),
                        ShowCloseButton::Hover => {
                            IconButton::new("关闭标签页", IconName::Close).visible_on_hover("")
                        }

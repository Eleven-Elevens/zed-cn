...
impl ThreadView {
        ...
        let (error_kind, acp_error_code, message): (&str, Option<SharedString>, SharedString) =
            match error {
                ThreadError::PaymentRequired => (
                    "payment_required",
                    None,
                    "你已达到免费用量限制。升级到 Zed Pro 可获得更多提示额度。"
                        .into(),
                ),
                ThreadError::Refusal => {
    ...
    pub fn render_thread_retry_status_callout(&self) -> Option<Callout> {
        ...
        let retry_message = if state.max_attempts == 1 {
            if next_attempt_in_secs == 1 {
                "正在重试。1 秒后再次尝试。".to_string()
            } else {
                format!("正在重试。{next_attempt_in_secs} 秒后再次尝试。")
            }
        } else if next_attempt_in_secs == 1 {
            format!(
                "正在重试。1 秒后再次尝试（第 {} 次，共 {} 次）。",
                state.attempt, state.max_attempts,
            )
        } else {
            format!(
                "正在重试。{next_attempt_in_secs} 秒后再次尝试（第 {} 次，共 {} 次）。",
                state.attempt, state.max_attempts,
            )
        };
    ...
    fn render_edits_summary(
        ...
        const EDIT_NOT_READY_TOOLTIP_LABEL: &str = "请等待文件编辑完成。";
                            ...
                            h_flex()
                                ...
                                .when(!is_done, |this| {
                                    this.child(
                                        IconButton::new("stop_subagent", IconName::Stop)
                                            ...
                                            .tooltip(Tooltip::text("停止子 Agent"))
                                ...
                                .child(
                                    IconButton::new("minimize_subagent", IconName::Dash)
                                        .icon_size(IconSize::Small)
                                        .tooltip(Tooltip::text("最小化子 Agent"))
    ...
    fn render_fast_mode_control(&self, cx: &mut Context<Self>) -> Option<AnyElement> {
        ...
        let (tooltip_label, color, icon) = if matches!(thread.speed(), Some(Speed::Fast)) {
            ("关闭快速模式", Color::Accent, IconName::FastForward)
        } else {
            (
                "启用快速模式",
    ...
    fn render_effort_selector(
        ...
        let label = selected
            ...
            .map_or("选择推理强度".into(), |effort| effort.name);
    ...
    fn render_send_button(&self, cx: &mut Context<Self>) -> AnyElement {
        ...
        if self.is_loading_contents {
            div()
                ...
                .px_1()
                .tooltip(Tooltip::text("正在加载添加的上下文…"))
            ...
            v_flex()
                ...
                .child(
                    h_flex()
                        ...
                        .child(Divider::horizontal())
                        .tooltip(Tooltip::text("下方所有内容均作为此子 Agent 的输出发送给主 Agent。")),
                )
                .child(primary)
                .into_any_element()
        } else {
        ...
        let last_turn_tokens_label = last_turn_clock
            ...
            .then(|| {
                self.turn_fields
                    ...
                    .map(|tokens| {
                        Label::new(format!("{} 个 token", crate::humanize_token_count(tokens)))
                            .size(LabelSize::Small)
                            .color(Color::Muted)
                    })
            })
                    ...
                    .child(
                        IconButton::new("feedback-thumbs-up", IconName::ThumbsUp)
                            ...
                            .tooltip(move |window, cx| match feedback {
                                ...
                                _ => {
                                    Tooltip::with_meta("回复有帮助", None, tooltip_meta(), cx)
                                }
                            })
                            .on_click(cx.listener(move |this, _, window, cx| {
            ...
            .when_some(turn_tokens_label, |this, tokens| {
                this.child(
                    h_flex()
                        ...
                        .child(
                            Label::new(format!("{} 个 token", tokens))
                                .size(LabelSize::Small)
                                .color(Color::Muted),
                        ),
                )
            ...
            .when(truncated_output, |header| {
                let tooltip = if let Some(output) = output {
                    if output_line_count + 10 > terminal::MAX_SCROLL_HISTORY_LINES {
                       format!("输出超过终端最大行数，已截断；模型收到前 {}。", format_file_size(output.content.len() as u64, true))
                    } else {
                        format!(
...
;
    ...
    fn render_subagent_card(
        ...
        let icon = h_flex().w_4().justify_center().child(if is_running {
            ...
            div()
                ...
                .tooltip(Tooltip::text("子 Agent 已取消"))
                .into_any_element()
        } else if is_failed {
            div()
                .id(status_icon)
                ...
                .tooltip(Tooltip::text("子 Agent 失败"))
                .into_any_element()
        } else {
            Icon::new(IconName::Check)
                .size(IconSize::Small)
        ...
        v_flex()
            ...
            .when_some(thread_view, |this, thread_view| {
                ...
                let fullscreen_toggle = h_flex()
                    ...
                    .tooltip(Tooltip::text("让子 Agent 全屏"))
    ...
    fn render_prompt_too_large_error(&self, cx: &mut Context<Self>) -> Callout {
        const MESSAGE: &str = "此对话已超出模型的上下文窗口。请新建线程或移除部分附件后继续。";
    ...
    fn render_resume_notice(_cx: &Context<Self>) -> AnyElement {
        let description = "此 Agent 不支持查看历史消息，但会从上次离开的地方继续会话。";
        ...
        self.skill_loading_errors
            ...
            .map(|(index, error)| {
                ...
                Callout::new()
                    ...
                    .actions_slot(
                        Button::new(("open-skill-file", index), "打开文件").on_click(cx.listener(
                            move |_, _, window, cx| {
    ...
    fn render_external_source_prompt_warning(&self, cx: &mut Context<Self>) -> Callout {
        Callout::new()
            ...
            .severity(Severity::Warning)
            .title("发送前审查")
            .description("此提示已由外部链接预先填充。发送前请仔细阅读。")
            .dismiss_action(
    ...
    fn render_multi_root_callout(&self, cx: &mut Context<Self>) -> Option<Callout> {
        ...
        let description = format!(
            "此 Agent 仅在“{}”中运行。它无法访问此工作区中的其他文件夹。",
            active_dir
        );
    ...
    fn render_token_limit_callout(&self, cx: &mut Context<Self>) -> Option<Callout> {
        ...
        let description = "若要继续，请从摘要开始新线程。";

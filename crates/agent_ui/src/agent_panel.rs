    ...
    fn title(&self, cx: &App) -> SharedString {
        ...
        if let Some(custom_title) = view.custom_title() {
            ...
            if terminal.breadcrumb_text.is_empty() {
                ...
                if title.eq_ignore_ascii_case("terminal") {
                    SharedString::from("")
                } else {
                    title.into()
                }
...
impl AgentPanel {
    ...
    fn load_thread_from_clipboard(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        if !self.has_open_project(cx) {
            Self::show_deferred_toast(&self.workspace, "打开项目以加载线程", cx);
            return;
        }

        let Some(clipboard) = cx.read_from_clipboard() else {
            Self::show_deferred_toast(&self.workspace, "剪贴板没有可用内容", cx);
            return;
        };
    ...
    fn show_thread_metadata(
        ...
        let Some(thread_id) = self.active_thread_id(cx) else {
            Self::show_deferred_toast(&self.workspace, "没有活动线程", cx);
            return;
        };
...
impl AgentPanel {
                ...
                move |window, cx| {
                    ...
                    Some(ContextMenu::build(window, cx, |mut menu, _window, _| {
                        ...
                        if !showing_terminal {
                            ...
                            if !skills_enabled {
                                menu = menu.action("规则", Box::new(OpenRulesLibrary::default()));
                            }

                            menu = menu.action("配置档案", Box::new(ManageProfiles::default()));
                        }
    ...
    fn render_toolbar_back_button(&self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        IconButton::new("go-back", IconName::ArrowLeft)
            ...
            .tooltip({
                move |_window, cx| {
                    Tooltip::for_action_in("后退", &workspace::GoBack, &focus_handle, cx)
                }
            })
    }

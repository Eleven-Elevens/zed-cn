...
impl Render for KeyContextView {
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl ui::IntoElement {
        ...
        v_flex()
            ...
            .child(Label::new("此视图可帮助你确定当前上下文栈，以便在 Zed 中创建自定义键位绑定。触发键盘快捷键时，它还会显示所有可能触发的上下文，以及实际匹配的上下文。"))
            .child(
                h_flex()
                    .mt_4()
                    .gap_4()
                    ...
                    .child(
                        Button::new("view_default_keymap", "查看默认Keymap")
                            .style(ButtonStyle::Filled)
            ...
            .when_some(key_equivalents, |el, key_equivalents| {
                el.child(Label::new("等效键").mt_4().size(LabelSize::Large))
                    .child(Label::new("使用某些字符定义的快捷键已被重新映射，因此无需按住 Option 也能输入这些快捷键。"))
                    .children(
                        key_equivalents
                            .iter()
                            .sorted()
